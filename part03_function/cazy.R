...
############
data <- myread("../01.abun/cazy_result.txt2")
...
#save(dt_norm,file="cazy.known.norm.RData")
...
data3 <- data2 %>%
  mutate(pathway = str_extract(name, "^[A-Za-z]+")) 
...
result <- data4 %>%
  mutate(average = rowMeans(select(., -pathway), na.rm = TRUE)) %>% 
  select(pathway, average)
sum(result$average, na.rm = TRUE)

labels <- result$pathway
...
pie_colors <- custom_colors[result$pathway]
...
###############
gene.map <- read.table("../01.abun/cazy.map", header = F, fill = TRUE, sep = "\t",check.names = F)
gene.map_clean <- gene.map %>%
  mutate(
    # 提取所有匹配的家族编号（GH, GT, PL, CE, CBM, AA 后面跟数字）
    CAZy_family = str_extract_all(V2, "\\b(GH|GT|PL|CE|CBM|AA)\\d+") %>%
      # 如果有多个家族（如 CBM50|GH25），用 "|" 连接
      sapply(function(x) paste(x, collapse = "|"))
  ) %>%
  # 只保留 V1 和新提取的 CAZy_family 列
  select(V1, CAZy_family)
gene.map_clean2 <- gene.map_clean %>%
  separate_longer_delim(CAZy_family, delim = "|")

data5 <- data3[,c(1,ncol(data3))]
merge2 <- merge(gene.map_clean2,data5,by.x = "CAZy_family",by.y = "name")

taxo <- myread("/share/data1/limin/vagina/mmseq/taxon/geneset.taxonomy")
taxo <- rownames_to_column(taxo,var="name")
merge3 <- merge(merge2,taxo[,c(1,7)],by.x = "V1",by.y = "name")
